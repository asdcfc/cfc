...
class UserController extends BaseController {
    ...
    {
        if(IS_POST){
            $sql = "select user_name,mobile,recommender,open_acount,open_name,open_bank,id_card,regis_time from tp_new_user limit 1,20";
            $data = M()->query($sql);
            $return['state'] = 1;
            $return['data'] = $data;
            // var_dump($data);die;
    ...
    public function ajaxindex(){
        ...
        I('user_name') ? $condition['user_name'] = I('user_name') : false;
        I('recommender') ? $condition['recommender'] = I('recommender') : false;
        ...
        {
            $first_leader = M('new_user')->query("select first_leader,count(1) as count  from __PREFIX__users where first_leader in(".  implode(',', $user_id_arr).")  group by first_leader");
            $first_leader = convert_arr_key($first_leader,'first_leader');
            
            $second_leader = M('new_user')->query("select second_leader,count(1) as count  from __PREFIX__users where second_leader in(".  implode(',', $user_id_arr).")  group by second_leader");
            $second_leader = convert_arr_key($second_leader,'second_leader');            
            
            $third_leader = M('new_user')->query("select third_leader,count(1) as count  from __PREFIX__users where third_leader in(".  implode(',', $user_id_arr).")  group by third_leader");
    ...
    public function detail(){
        $uid = I('get.id');
        $user = D('new_user')->where(array('user_id'=>$uid))->find();
        if(!$user)
            exit($this->error('会员不存在'));
        if(IS_POST){
            //  会员信息编辑

            $row = M('new_user')->where(array('user_id'=>$uid))->save($_POST);
            if($row)
    ...
    public function delete(){
        $uid = I('get.id');
        $row = M('new_user')->where(array('user_id'=>$uid))->delete();
        if($row){
            $this->success('成功删除会员');
        }else{
            $this->error('操作失败');
